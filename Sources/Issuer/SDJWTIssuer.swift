...
public class SDJWTIssuer {
  ...
  static func createSDJWT<KeyType>(purpose: Purpose, signingKey: KeyType) throws -> SignedSDJWT {
    ...
    case .presentation(let signedJWT, let selectedDisclosures, let kbjwt):
      ...
      if let kbjwt {
          var finalKbjwt = kbjwt
          //Update KBJWT with sd_hash if possible
          if let jwtPayload = try signedJWT.jwt.payloadJSON().rawValue as? Dictionary<String, Any> {
              if let sdAlg = jwtPayload["_sd_alg"] as? String, let hashAlgID = HashingAlgorithmIdentifier(rawValue: sdAlg) {
                  let serializedStringRep = CompactSerialiser(signedSDJWT: signedJWT).serialised
                  if let hash = hashAlgID.hashingAlgorithm().hash(disclosure: serializedStringRep), var payloadDict = kbjwt.payload.dictionaryObject {
                      payloadDict["sd_hash"] = hash.base64URLEncode()
                      let kbJwtPayload = JSON(payloadDict)
                      let updatedKBJWT = try KBJWT(header: kbjwt.header, kbJwtPayload: kbJwtPayload)
                      finalKbjwt = updatedKBJWT
                  }
              }
          }
          
        return try createKeyBondedSDJWT(signedSDJWT: signedJWT, kbJWT: finalKbjwt, holdersPrivateKey: signingKey)
      }
      return signedJWT
      // ..........
    }
